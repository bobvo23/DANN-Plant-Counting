...
def main(config):
    ...
    print(f"Total testing files: {len(data_loader)*data_loader.batch_size}")
    with torch.no_grad():
        for i, (data, target) in enumerate(tqdm(data_loader)):
            data, target = data.to(device), target.to(device)
            class_pred, domain_pred = model(data, 0)
            ...
            loss = loss_fn(class_pred, target)
            batch_size = data.shape[0]
            total_loss += loss.item() * batch_size
            for i, metric in enumerate(metric_fns):
                total_metrics[i] += metric(class_pred, target) * batch_size
